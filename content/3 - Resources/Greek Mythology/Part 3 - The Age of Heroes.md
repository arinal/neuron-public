---
id: greek-mythology-chronological-3
aliases:
  - Greek Mythology Timeline Part III
  - The Age of Heroes
tags:
  - mythology
  - greek
...
```mermaid
graph TD
    IAP[Iapetus<br/>Titan] --> PRO[Prometheus]
    IAP --> EPI[Epimetheus]
    PRO -->|moulds from clay| CLAY[the CLAY-RACE<br/>→ wicked Bronze men<br/>✝ drowned in the Flood]
    PRO --- U1(( ))
    PRON[Pronoia<br/>a nymph] --- U1
    U1 --> DEU[Deucalion<br/>mortal, flood-survivor]
    EPI --- U2(( ))
    PAND[Pandora] --- U2
    U2 --> PYR[Pyrrha<br/>mortal, flood-survivor]
    DEU --- U3(( ))
    PYR --- U3
    DEU -->|his thrown stones → men| STONE[the STONE-RACE<br/>= the commons/*laoi*<br/>the Leleges of Locris, king Locrus]
    PYR -->|her thrown stones → women| STONE
    U3 --> HEL[Hellen<br/>eponym of the Hellenes]
    HEL --- U4(( ))
    ORS[Orseis<br/>a nymph] --- U4
    U4 --> AEO[Aeolus<br/>→ Aeolians]
    U4 --> DOR[Dorus<br/>→ Dorians]
    U4 --> XU[Xuthus]
    XU --> ACH[Achaeus<br/>→ Achaeans]
    XU --> ION[Ion*<br/>→ Ionians<br/>*truly son of Apollo]
    AEO --> AEOLIDS[the Aeolid houses<br/>Sisyphus · Athamas · Cretheus · Salmoneus<br/>→ Jason & the Argonauts]
    classDef union fill:transparent,stroke:none,color:transparent;
    class U1,U2,U3,U4 union;
```
...
graph TD
    ZEUS[Zeus] --- U1(( ))
    IO[Io<br/>heifer-maiden] --- U1
    U1 --> EPA[Epaphus<br/>King of Egypt, founds Memphis]
    EPA --> LIB[Libya]
    LIB --> BEL[Belus]
    BEL --> AEG[Aegyptus]
    BEL --> DAN[Danaus<br/>flees to Argos with the 50 Danaids]
    DAN --> LINE[...Argive kings...]
    LINE --> ACR[Acrisius<br/>king of Argos]
    ACR --> DANAE[Danaë]
    ZEUS --- U2(( ))
    DANAE --- U2
    U2 --> PERS[Perseus]
    PERS --> HERO[the hero-houses<br/>§1 onward]
    classDef union fill:transparent,stroke:none,color:transparent;
    class U1,U2 union;
```
...
graph TD
    ZEUS[Zeus] --- U1(( ))
    DAN[Danaë] --- U1
    ACR[Acrisius<br/>king of Argos] --> DAN
    U1 --> PERS[Perseus]
    PERS --- U2(( ))
    AND[Andromeda] --- U2
    PERS -->|accidentally kills| ACR
    U2 --> ELEC[Electryon]
    U2 --> STHEN[Sthenelus]
    ELEC --> ALCM[Alcmene]
    STHEN --> EURY[Eurystheus<br/>sets Heracles' Labours]
    ZEUS --- U3(( ))
    ALCM --- U3
    U3 --> HERAC[Heracles]
    classDef union fill:transparent,stroke:none,color:transparent;
    class U1,U2,U3 union;
```
...
graph TD
    ...
    ALC[Alcmene] --- U1
    U1 --> HER[Heracles]
    HERA[Hera] -->|lifelong persecution / madness| HER
    HER -->|12 Labours in service of| EUR[Eurystheus]
    HER -.marries then kills in madness.- MEG[Megara]
    HER -.later marries.- DEI[Deïaneira]
    DEI -->|Nessus' poisoned blood| DEATH[Death on Mt Oeta]
    DEATH --> APO[Apotheosis → god on Olympus]
    APO -.marries.- HEBE[Hebe]
    classDef union fill:transparent,stroke:none,color:transparent;
    class U1 union;
```
...
```mermaid
graph TD
    PEL[Pelias<br/>usurper of Iolcus] -->|sets impossible quest| JAS[Jason]
    JAS -->|captains| ARGO[the Argo + Argonauts]
    ARGO --- HERAC[Heracles]
    ARGO --- ORPH[Orpheus]
    ARGO --- DIOS[Castor & Pollux]
    ARGO --- PEL2[Peleus → father of Achilles]
    JAS -.aided & married.- MED[Medea of Colchis]
    MED -->|wins the| FLEECE[Golden Fleece]
    JAS -.abandons her → tragedy.- MED
```
...
```mermaid
graph TD
    AEG[Aegeus / Poseidon] --> THES[Theseus]
    MINOS[King Minos of Crete] --> ARI[Ariadne]
    PAS[Pasiphaë] --- U1(( ))
    BULL[Cretan Bull] --- U1
    U1 --> MINO[Minotaur]
    MINOS -->|imprisons in| LAB[the Labyrinth<br/>built by Daedalus]
    LAB --- MINO
    ARI -->|gives thread| THES
    THES -->|slays| MINO
    THES -.abandons.- ARI
    ARI -.then wed by.- DIO[Dionysus]
    THES -->|black sails → grief| AEG
    classDef union fill:transparent,stroke:none,color:transparent;
    class U1 union;
```
